...
def main():
    ...
    with st.sidebar:
        ...
        # 讀取 .env 的預設設定
        env_api_key = Config.MAINNET_API_KEY
        env_secret_key = Config.MAINNET_SECRET_KEY
        default_demo = Config.USE_DEMO
        
        # 網路選擇（以 .env 的 USE_DEMO 作為預設）
        ...
        # 若 .env 已配置金鑰，則不再要求輸入；否則提供輸入框
        if env_api_key and env_secret_key:
            st.success("已從 .env 載入 API 金鑰")
            # 遮罩顯示
            masked_api = f"{env_api_key[:3]}...{env_api_key[-3:]}" if len(env_api_key) >= 6 else "********"
            st.text_input("API Key", value=masked_api, type="password", disabled=True)
        ...
        if st.button("🔌 連接 API", type="primary"):
            ...
            if api_key and secret_key:
                ...
                st.error("請輸入 API Key 和 Secret Key，或於 .env 設定後重試")
